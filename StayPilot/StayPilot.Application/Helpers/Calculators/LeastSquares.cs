    ...
    public static class LeastSquares
    {
        ...
        public static LeastSquaresFit Fit(IReadOnlyList<double[]> rows, IReadOnlyList<double> targets)
        {
            ...
            for (var r = 0; r < rows.Count; r++)
            {
                ...
                {
                    // Most columns are dummy flags sitting at zero, so skipping them is a big win.
                    if (row[i] == 0)
                        continue;

                    normalVector[i] += row[i] * targets[r];
            ...
            // Residual variance: average leftover error, spread over the degrees of freedom.
            ...
            return new LeastSquaresFit
            {
                ...
                ResidualVariance = sumSquaredResiduals / (rows.Count - columns),
            };
        }
